...
def stock_realtime_humanity():
    ...
    def stock_data(stock_dict):
        f = open('dtss_humanity_result.txt', 'w')
        stock_data = {}
        for stock_num, stock in stock_dict.items():
            if stock['success']:
                ...
                try:#解決當天沒有交易的股票5/2
                    ...
                    if 1500<total_volume<20000 and 10<last_price<100 and total_volume !=None and 0.025<z<0.065 and -0.01<g<0.015:#從早上開執行
                        ...
                        lp="%.2f" % last_price
                        print('股號',stock_num,'\n''現價',lp,'均價',avg,'總成交量',total_volume,'單量',trade_volume)

                        f = open('dtss_humanity_result.txt', 'a')
                        #f.write('股號: '+stock_num+' 現價 '+last_price+' 均價 '+a+'\n')#
                        f.write('{{[[((--股號： '+str(stock_num)+' 現價： '+str(lp)+' 均價： '+str(avg)+'--))]]}}\n')
